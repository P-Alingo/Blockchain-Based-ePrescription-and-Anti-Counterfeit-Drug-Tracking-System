...
import { Activity, Shield, Search, FileText, AlertTriangle, CheckSquare, Cog, Settings, Users } from "lucide-react";
...
const AdminBlockchain = () => {
  ...
  const [error, setError] = useState("");
    ...
    useEffect(() => {
          ...
          console.log('[AdminBlockchain] Fetching /api/admin/blockchain');
          fetch(`${API_BASE}/api/admin/blockchain`, { headers })
            .then(async (res) => {
              if (!res.ok) {
            ...
            .then((data) => {
              // Defensive: handle array or object with logs property
   ...
   return (
     <DashboardLayout sidebarItems={sidebarItems} userRole="admin" userName="System Admin" userEmail="admin@eprescribe.go.ke">
       <div className="space-y-8">
         <div className="mb-6">
           <h1 className="text-3xl font-bold bg-gradient-to-r from-gray-700 to-gray-900 bg-clip-text text-transparent">Blockchain Event Logs</h1>
           <p className="text-lg text-muted-foreground mt-2">System-wide blockchain event records for audit, traceability, and compliance</p>
         </div>
         <div className="mb-8">
           <h2 className="text-xl font-bold bg-gradient-to-r from-gray-700 to-gray-900 bg-clip-text text-transparent mb-2">Recent Blockchain Events</h2>
         </div>
